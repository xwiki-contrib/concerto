 ...
 * @version $Id:$
 */
public class LastModifiedPageNameListTest extends AbstractWootEngineTest
{
    /**
     * Test the add and remove methods on the last modified pagename list in {@link PageManager}.
     * 
    ...
    public void pageListTest() throws WootEngineException
    {
        ...
        Assert.assertEquals(pn1, this.site0.getPageManager().getLastModifiedPageNameList().getCurrentPageNameList()
            .get(0));
        ...
        this.site0.getPageManager().getLastModifiedPageNameList().addPageNameInList(pn1);
        Assert.assertEquals(5, this.site0.getPageManager().getLastModifiedPageNameList().getCurrentPageNameList()
            .size());

        this.site0.getPageManager().getLastModifiedPageNameList().removeNOccurencesInList(pn1, 2);
        Assert.assertEquals(pn2, this.site0.getPageManager().getLastModifiedPageNameList().getCurrentPageNameList()
            .get(0));
        Assert.assertEquals(pn3, this.site0.getPageManager().getLastModifiedPageNameList().getCurrentPageNameList()
            .get(1));
        Assert.assertEquals(pn1, this.site0.getPageManager().getLastModifiedPageNameList().getCurrentPageNameList()
            .get(2));
    }

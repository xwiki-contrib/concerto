...
public class MultiplePeersTest extends AbstractJxtaTestBase
{
    ...
    {
        TestCaseLauncher.launchTest(DiscoverPeersInGroup.class.getName(), 2);
    }

    @Test
    public void testBroadcastMessageToGroup() throws Exception
    {
        TestCaseLauncher.launchTest(BroadcastMessageToGroup.class.getName(), 2);
    }

    /*private Object[] launchTest(String className, int numberOfPeers) throws Exception
    {
        ...
        for (int i = 0; i < numberOfPeers; i++) {
            ...
            Boolean inited = (Boolean) initMethodTestCase[i].invoke(testCases[i], initParameters);
            ...
            Boolean connected = (Boolean) connectMethodTestCase[i].invoke(testCases[i], VOID_PARAMETERS);
            ...
            Boolean started = (Boolean) startMethodTestCase[i].invoke(testCases[i], startParameters);
